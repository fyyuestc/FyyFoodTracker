...
class MealTableViewController: UITableViewController {
    ...
    private func loadDefaltMeals(){
        ...
        guard let meal1 = Meal(name: "Beijing Roast Duck", photo: photo1, rating: 3) else {
            fatalError("不能加载北京烤鸭")
        }
        guard let meal2 = Meal(name: "Tian Tian Quan", photo: photo2, rating: 2) else {
            fatalError("不能加载甜甜圈")
        }
        guard let meal3 = Meal(name: "Langoustine", photo: photo3, rating: 4) else {
            fatalError("不能加载小龙虾")
        }
